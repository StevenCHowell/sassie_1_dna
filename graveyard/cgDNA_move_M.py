...
# sh        write_xyz('cg_dna.xyz',coor,comment,frame)
    ...
    for i in xrange(cg_dna.natoms()):
        ...
        infile.write("%-6s%5s %-4s%1s%-4s%1s%4s%1s   %8s%8s%8s%6s%6s      %-4s%2s%2s\n" % (cg_dna.atom()[i], this_index, cg_dna.name()[i], cg_dna.loc()[i], cg_dna.resname()[
                     i], cg_dna.chain()[i], cg_dna.resid()[i], cg_dna.rescode()[i], sx, sy, sz, cg_dna.occupancy()[i], cg_dna.beta()[i], cg_dna.segname()[i], cg_dna.element()[i], cg_dna.charge()[i]))
...
def make_cgDNA_model(all_atom_pdb, chain1, chain2, resid1, resid2, flexResid, bp_perBead):
    ...
    # check the input
    assert np.abs(resid1[1] - resid1[0]) == np.abs(resid2[1] -
                                                   resid2[0]), "number of paired bases in chains are not the same"
    ...
    s = 0
    i = 0
    # looping because selecting nbeads+1 bases did not always have N=nbeads
    # 'P' atoms
    while s < nbeads:
        # ; print 'bf = ',basis_filter
        basis_filter = "((name[i] == 'P') and (resid[i] < " + \
            str(nbeads + i) + "))"
        error, mask = aa_dna.get_subset_mask(basis_filter)  # ; print mask
        s = np.sum(mask)
        i += 1

    # pick out the first N=nbeads 'P' atoms
    error = aa_dna.copy_molecule_using_mask(cg_dna, mask, frame)
    # initialize array to store the coordinates of the beads
    cg_coor = np.zeros((1, nbeads, 3))

    r1a = resid1[0]
    r2a = resid2[0]
    # list to contain all the all atom sasmol object of each bead
    allBeads = []
    ...
    for j in xrange(nbeads):
        ...
        if resid1[0] < resid1[1]:
            ...
            basis_filter1 = "((resid[i] > " + str(r1a - 1) + " and resid[i] < " + str(
                r1b) + ") and (chain[i]=='" + chain1 + "')) or "
        else:
            r1b = r1a - bp_perBead  # r1b < r1a
            basis_filter1 = "((resid[i] > " + str(r1b) + " and resid[i] < " + str(
                r1a + 1) + ") and (chain[i]=='" + chain1 + "')) or "
        if resid2[0] < resid2[1]:
            r2b = r2a + bp_perBead  # r2b > r2a
            basis_filter2 = "((resid[i] > " + str(r2a - 1) + " and resid[i] < " + str(
                r2b) + ") and (chain[i]=='" + chain2 + "'))"
        else:
            r2b = r2a - bp_perBead   # r2b < r2a
            basis_filter2 = "((resid[i] > " + str(r2b) + " and resid[i] < " + str(
                r2a + 1) + ") and (chain[i]=='" + chain2 + "'))"
        ...
        # name = 'bead'+str(j)+'.pdb' ; bead.write_pdb(name,0,'w')
        # # output each bead to a pdb for verification purposes

        com = bead.calccom(frame)
        # ; print 'com ', j, '= ', com       # store the coordinates of the com for the bead coordinate
        cg_coor[0, j, :] = com

        # ; print   'bead._com = ', bead._com    # calculate the coodinates of the atoms in the bead with reference to the bead com
        bead.center(0)
...
def recover_aaDNA_model(cg_dna, aa_dna, vecXYZ, allBeads, masks):
    ...
    for i in xrange(cg_natoms):
        ...
        # R is the matrix that would align the rotated coordinates back to the
        # original
        R = align2xyz(vecX[i, :], vecY[i, :], vecZ[i, :])  # ; print 'R = ', R
        # M will rotate cooridates from the original reference to the rotated
        # coordinates of the bead then translate the com to the beads com
        M = R.transpose()
        # ; print 'M = ', M  # put the translation to the new com into the matrix
        M[3, :3] = coor[i, :]

        (r, c) = allBeads[i].coor()[0].shape
        # initialize the beadCoor matrix
        beadCoor = np.ones((r, 4))
        # ; print 'beadCoor (before) = \n',beadCoor
        beadCoor[:, :3] = allBeads[i].coor()[0]
        # ; print 'beadCoor (after) = \n',beadCoor
...
def align2xyz(vecX, vecY, vecZ):
    ...
    assert newX[
        2] < 10e-5, "ERROR!!! z-component of newX is not zero and it should be"

    # ;  print 'thetaZ_x = ', thetaZ_x
    thetaZ_x = -np.arctan2(newX[1], newX[0])
    # thetaZ_y = -np.arctan2(newY[1],-newY[0])  ;#  print 'thetaZ_y = ',
    # thetaZ_y
    ...
    assert newY[0] + newY[2] < 1 + \
        10e-5, "ERROR!!! newY is not aligned to the y-axis and it should be"
...
def beadRotate(coor3, vecXYZ, M, thetas, nSoft):
    ...
    # add a fourth column of ones to each bead origin and orientation vectors
    # (this incorporates translations)
    (natoms, col) = coor3.shape

    # initialize vector arrays for coordinates and orientation vectors making
    # them into 4 component vectors
    ...
    (R[2][0], R[2][1], R[2][2]) = (
        sx * sz + cx * sy * cz, cx * sy * sz - sx * cz, cx * cy)  # ; print 'R:\n', R
    ...
    # ; print "M (end of 'beadRotate)\n", M
    M[:] = np.dot(np.dot(M[:], A.transpose()), Ti0)
    ...
    # first (reference) bead
    return (coor4[1:, 0:3], vecXYZ[1:], M[1:])
...
def checkU(coor):
    ...
    erLim = 1e-3
    (r, c) = coor.shape
    # u-vectors pointing from bead to bead
    u = coor[1:, :] - coor[:r - 1, :]
    # magnitues of u-vectors -> distance btwn beads
    ...
    # check if any of the l-lengths are different
    test = np.abs(lu - l) > erLim
    # s if test.any():
...
def energyWCA(w, coor, wca0, trial_bead):
    ...
    for i in xrange(trial_bead, N):
        ...
        for j in xrange(0, i):
            rj = coor[j, :]
            rij = np.sqrt(
                (ri[0] - rj[0])**2. + (ri[1] - rj[1])**2. + (ri[2] - rj[2])**2.)
...
def dna_mc(nsteps, cg_dna, vecXYZ, lp, w, theta_max, trialbeads, M, nSoft=3, f=True):
    # def
    # dna_mc(nsteps,cg_dna,vecXYZ,lp,w,theta_max,trialbeads,nSoft=3,f=True):
    '''
    this function perform nsteps Monte-Carlo moves on the cg_dna
    '''
    dcdOutFile = cg_dna.open_dcd_write("cg_dna_moves.dcd")

    # need to change this so there could be components that are not the beads
    nbeads = cg_dna.natoms()
    nflex = trialbeads.size
    xyz = np.copy(vecXYZ)
    coor = np.copy(cg_dna.coor()[0])
    Mtmp = np.copy(M)
    # get the vectors pointing from bead to bead and make sure they are
    # equidistant
    (u, l) = checkU(coor)
    ...
    for i in xrange(nsteps):
        ...
        thetaZ_max = np.float(
            theta_max) / 10.  # this should be something small
        ...
        # ; print 'thetaXYZ = ', thetaXYZ
        thetaXYZ = [thetaX / nSoft, thetaY / nSoft, thetaZ / nSoft]

        # s print 'coor before:\n',coor
        (coor[trial_bead:], xyz[trial_bead:], Mtmp[trial_bead:]) = beadRotate(coor[trial_bead - 1:],
                                                                              xyz[trial_bead - 1:], Mtmp[trial_bead - 1:], thetaXYZ, nSoft)  # generate a newly rotated model
        # s print 'coor after\n',coor

        # calculate the change in energy (dU) and boltzman factor (p) for the
        # new model
        (u, l) = checkU(coor)
        ...
        # if True:
        if test < p:
            # print 'wrote new dcd frame (end of loop',i,'
            # trial_bead=',trial_bead,')'+' accepted new configuration\n'
            a += 1
            ...
            # print 'wrote new dcd frame (end of loop',i,'
            # trial_bead=',trial_bead,')'+' rejected new configuration\n'
            r += 1
...
if __name__ == "__main__":
    ...
    (cg_dna, aa_dna, vecXYZ, allBeads, trialbeads, masks, M) = make_cgDNA_model(
        all_atom_pdb, chain1, chain2, resid1, resid2, flexid, bp_perBead)
    ...
    if show:
        ...
        ax = Axes3D(fig)
        ax.scatter(
            cg_dna.coor()[0, :, 0], cg_dna.coor()[0, :, 1], cg_dna.coor()[0, :, 2])
        ax.set_xlabel('X')
        ax.set_ylabel('Y')
        ax.set_zlabel('Z')
    ...
    for i in xrange(iters):
        (cg_dna, vecXYZ, a, r, M) = dna_mc(
            nsteps, cg_dna, vecXYZ, lp, w, theta_max, trialbeads, M, nSoft, f)
        # s print 'cg_dna.coor() =\n', cg_dna.coor()
        ...
        if show:
            ...
            ax.set_ylabel('Y')
            ax.set_zlabel('Z')
            fig.suptitle('After %d steps Rg/lp=%f  Re/lp=%f' %
                         ((i + 1) * nsteps, rg_lp[i], re_lp[i]))
    ...
    error, aa_dna, Mreal = recover_aaDNA_model(
        cg_dna, aa_dna, vecXYZ, allBeads, masks)
