...
class Drv():
    ...
    def verify_me(self):
        ...
        result_files = [os.path.split(result_files[i])[1]
                        for i in range(len(result_files))]
        expected_files = glob.glob(expected_path + '*')
        expected_files = [
            os.path.split(expected_files[i])[1] for i in range(len(expected_files))]
        ...
        for ifile in expected_files:
            if ifile in result_files:
                ...
                if ifile[-4:] == '.log':
                    flag = (flag and FileCmp.cmp_skip(
                        expected_path + ifile, result_path + ifile, [1, 4, 23, 58]))
                else:
                    flag = (
                        flag and filecmp.cmp(expected_path + ifile, result_path + ifile))
                print '\n...to be...', flag
                if flag == False:
                    return False
            else:
