...
def main():
    ...
    basis_filter = "((segname[i] == '%s') and (name[i] == 'CA') and (resid[i] >= %s) and (resid[i] <= %s))" % (
        match_segname, match_res_min, match_res_max)
    ...
    for i in xrange(n_frames):
        ...
        a2.write_frame_to_file(
            aa_move, n_frames, i + 1, path, save_file, out_type, dcd_out_file, in_type)
