...
def mask_method(ncp1_c1p_mask, ncp1_dyad_resids, ncp2_c1p_mask, ncp2_dyad_resids,
    ...
    for frame in xrange(n_frames):
        ...
        if not aligned or frame is 0:
            ...
            ncp1_x, ncp1_y, ncp1_z = geometry.get_axes_from_points(
                ncp1_origin, ncp1_x_coor, ncp1_y_coor)
        ...
        ncp2_x, ncp2_y, ncp2_z = geometry.get_axes_from_points(
            ncp2_origin, ncp2_x_coor, ncp2_y_coor)
...
def get_closest_atoms(dimer_dcdfile, dimer, aligned, ncp1_c1p_mask, ncp1_dyad_resids,
    ...
    ncp1_x_axis_vectors = geometry.vector_from_line(
        dimer.coor()[0], ncp1_origin, ncp1_axes[0])
    ncp1_x_axis_magnitudes = np.sqrt(
        inner1d(ncp1_x_axis_vectors, ncp1_x_axis_vectors))
    print_min(ncp1_x_axis_magnitudes, dimer, 'ncp1_x_axis')
    i_min_ncp1_x = ncp1_x_axis_magnitudes.argmin()
    ncp1_y_axis_vectors = geometry.vector_from_line(
        dimer.coor()[0], ncp1_origin, ncp1_axes[1])
    ncp1_y_axis_magnitudes = np.sqrt(
        inner1d(ncp1_y_axis_vectors, ncp1_y_axis_vectors))
    print_min(ncp1_y_axis_magnitudes, dimer, 'ncp1_y_axis')
    i_min_ncp1_y = ncp1_y_axis_magnitudes.argmin()
    ncp1_z_axis_vectors = geometry.vector_from_line(
        dimer.coor()[0], ncp1_origin, ncp1_axes[2])
    ncp1_z_axis_magnitudes = np.sqrt(
        inner1d(ncp1_z_axis_vectors, ncp1_z_axis_vectors))
    print_min(ncp1_z_axis_magnitudes, dimer, 'ncp1_z_axis')
    i_min_ncp1_z = ncp1_z_axis_magnitudes.argmin()

    ncp2_x_axis_vectors = geometry.vector_from_line(
        dimer.coor()[0], ncp2_origin, ncp2_axes[0])
    ncp2_x_axis_magnitudes = np.sqrt(
        inner1d(ncp2_x_axis_vectors, ncp2_x_axis_vectors))
    print_min(ncp2_x_axis_magnitudes, dimer, 'ncp2_x_axis')
    i_min_ncp2_x = ncp2_x_axis_magnitudes.argmin()
    ncp2_y_axis_vectors = geometry.vector_from_line(
        dimer.coor()[0], ncp2_origin, ncp2_axes[1])
    ncp2_y_axis_magnitudes = np.sqrt(
        inner1d(ncp2_y_axis_vectors, ncp2_y_axis_vectors))
    print_min(ncp2_y_axis_magnitudes, dimer, 'ncp2_y_axis')
    i_min_ncp2_y = ncp2_y_axis_magnitudes.argmin()
    ncp2_z_axis_vectors = geometry.vector_from_line(
        dimer.coor()[0], ncp2_origin, ncp2_axes[2])
    ncp2_z_axis_magnitudes = np.sqrt(
        inner1d(ncp2_z_axis_vectors, ncp2_z_axis_vectors))
    ...
    ncp1_origin_magnitudes = np.sqrt(
        inner1d(ncp1_origin_vectors, ncp1_origin_vectors))
    ...
    ncp2_origin_magnitudes = np.sqrt(
        inner1d(ncp2_origin_vectors, ncp2_origin_vectors))
...
def fit_method(n_frames, dimer_dcdfile, dimer, aligned, ncp1_c1p_mask,
    ...
    ncp1_dyad_filter = '( segname[i] == "%s" and resid[i] == %d ) or ( segname[i] == "%s" and resid[i] == %d )' % (
        dna_ids[0], ncp1_dyad_resids[0], dna_ids[1], ncp1_dyad_resids[1])
    ncp2_dyad_filter = '( segname[i] == "%s" and resid[i] == %d ) or ( segname[i] == "%s" and resid[i] == %d )' % (
        dna_ids[0], ncp2_dyad_resids[0], dna_ids[1], ncp2_dyad_resids[1])
    ...
    for frame in xrange(n_frames):
        ...
        dimer.read_dcd_step(dimer_dcdfile, frame)
        coor = dimer.coor()
        coor[0] = geometry.transform_coor(
            coor[0], np.array([1, 0, 0]), np.array([0, 0, 0]))
        dimer.setCoor(coor)
...
def load_x2file(filename, n_frames=None):
    data = pd.read_table(
        filename, sep='\t', skiprows=2, header=None, nrows=n_frames)
