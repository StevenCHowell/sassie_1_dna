...
def show_ncps(all_ncp_plot_vars, title='NCP array'):
    ...
    for i in xrange(n_ncps):
        ...
        if i_ncp == n_ncps:
            ...
            ax.plot(
                dyad_origin[:, 0], dyad_origin[:, 1], dyad_origin[:, 2], 'rs')
...
def get_dna_bp_and_axes_slow(dna_resids, dna_ids, dna_mol, dna_id_type='segname'):
    bp_filter = '( %s[i] == "%s" and resid[i] == %d ) or ( %s[i] == "%s" and resid[i] == %d )' % (
        dna_id_type, dna_ids[0], dna_resids[0], dna_id_type, dna_ids[1], dna_resids[1])
...
def get_ncp_origin_and_axes(ncp_c1p_mask, dyad_mask, dyad_dna_id, ncp, prev_opt_params=None,
    ...
    dyad_origin, dyad_axes, dyad_mol = get_dna_bp_and_axes(
        dyad_mask, dyad_dna_id, ncp, dyad_mol, dna_id_type)
    ...
    if debug:
        ...
        show_cylinder(
            coor, opt_params, ncp_origin, ncp_axes, dyad_origin, dyad_axes)
...
if __name__ == '__main__':
    ...
    ncp_origin, ncp_axes, opt_params, dyad_mol = get_ncp_origin_and_axes(
