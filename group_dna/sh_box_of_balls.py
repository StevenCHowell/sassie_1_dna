...
def check_ball_overlap(single_molecule, this_group, group_to_rotate, cutoff):
    ...
    error, fc_mask = single_molecule.get_subset_mask(fc_basis)
    check_error(error)
    error, fab1_mask = single_molecule.get_subset_mask(fab1_basis)
    check_error(error)
    error, fab2_mask = single_molecule.get_subset_mask(fab2_basis)
    check_error(error)

    error, fc_coor = single_molecule.get_coor_using_mask(frame, fc_mask)
    check_error(error)
...
def get_com_coords_and_build_box(single_molecule, coor_start_type, number_of_molecules, argon_file_name, argon_box_length, length_scale_factor, mab_diameter):
    ...
    max_diameter_molecule = max(dx, dy, dz)
    print 'maximum molecular diameter = ', max_diameter_molecule

    if(mab_diameter > 0.0):
        min_diameter_molecule = mab_diameter
    else:
        min_diameter_molecule = min(dx, dy, dz)
        print 'minimum molecular diameter = ', min_diameter_molecule

    print 'max diameter = ', max_diameter_molecule
    print 'min diameter = ', min_diameter_molecule

    if(coor_start_type == 1):
        com_coor, boxl = setup_mmc.makefcc(
            2.0 * max_diameter_molecule, number_of_molecules)
...
def duplicate_molecule_and_assign_coor(all_molecules, single_molecule, com_coor, initial_pdb, number_of_steps, groups, group_masks, number_of_groups, residues_in_groups, rotation_variables, dihedral_parameters, flexible_residues, residue_rotation_indices, residue_rotation_mask, step_parameters, first_last_resid, cutoff):
    ...
    error = single_molecule.copy_molecule_using_mask(
        original_single_molecule, all_atoms_mask, frame)
    check_error(error)
    ...
    for i in xrange(number_of_duplicates):
        ...
        if(i > 0):
            ...
            # note that the check_error(error) merely prints out the error and
            # quits program

            # grab the current mask for the balls-only from the complete system

            error, growing_mask = growing_molecule.get_subset_mask(ball_basis)
            check_error(error)

            # get the coordinates of the balls-only from the complete system

            error, coor1 = growing_molecule.get_coor_using_mask(
                frame, growing_mask)
            check_error(error)
            ...
            while(check == 1):
                ...
                # are randomly rotated about an arbitrary axis (x,y,or z) and
                # the COM is moved to this_com

                # SHOULD MERELY COPY SINGLE MOLECULE TO TRIAL MOLECULE ONCE
                # THEN REASSIGN COORDS AND ROTATE HERE

                zero = numpy.array([0.0, 0.0, 0.0])
                #error = single_molecule.duplicate_molecule(trial_molecule,1,frame,zero) ; check_error(error)
                error = single_molecule.copy_molecule_using_mask(
                    trial_molecule, all_atoms_mask, frame)
                check_error(error)

                rotate_single_mab(single_molecule, trial_molecule, groups, group_masks, number_of_groups, residues_in_groups, rotation_variables,
                                  dihedral_parameters, flexible_residues, residue_rotation_indices, residue_rotation_mask, step_parameters, first_last_resid, cutoff)
                ...
                trial_molecule.write_dcd_step(
                    dcdoutfile2, 0, number_accepted_structures + 1)
                ...
                error, coor2 = trial_molecule.get_coor_using_mask(
                    frame, ball_mask)
                check_error(error)

                # check for overlap of the balls between the complete system
                # and the trial_molecule

                check = poverlap.faboverlap(coor1[0], coor2[0], cutoff)
                count += 1

                if(check == 1 and count == max_count):
                    print '>> need to translate and rotate a ball'
                    ntr += 1
                    save_coor = False
                    check = 0
                    count = 0
            ...
            if save_coor:
                ...
                growing_molecule.merge_two_molecules(
                    last_molecule, trial_molecule)

        else:
            error = single_molecule.duplicate_molecule(
                trial_molecule, 1, frame, this_com)
            check_error(error)
...
def get_linker_assign_coor(three_ball, ball_diameter, minimum_base_angle, surface1_to_anchor_cord_length, linker_pdb):
    ...
    linker.moveto(frame, [0.0, 0.0, (ball_diameter / 2.0) +
                          surface1_to_anchor_cord_length + (linker_length / 2.0)])
...
def angle_initialization(hybrid, rotation_variables):
    ...
    dihedral_parameters, flexible_residues, residue_rotation_indices, residue_rotation_mask, step_parameters, first_last_resid = initialize_rotation.initialize_system(
        hybrid, molecule_type, basis, number_of_ranges, reslow, numcont)
...
def main(simulation_type, number_of_steps, initial_pdb, output_pdb, argon_file_name, number_of_molecules, argon_box_length, length_scale_factor, ball_diameter, fc_to_fab_vector_length, minimum_base_angle, surface1_to_anchor_cord_length, linker_pdb, rotation_variables, number_of_groups, residues_in_groups, mab_diameter):
    ...
    if(start_type != 1):
        ...
        three_ball = build_ball_coordinates(
            ball_diameter, fc_to_fab_vector_length, minimum_base_angle)

        single_molecule = get_linker_assign_coor(
            three_ball, ball_diameter, minimum_base_angle, surface1_to_anchor_cord_length, linker_pdb)

    single_molecule._totalmass = 150000
    print 'hybrid mass (amu) = ', single_molecule.totalmass()

    dihedral_parameters, flexible_residues, residue_rotation_indices, residue_rotation_mask, step_parameters, first_last_resid = angle_initialization(
        single_molecule, rotation_variables)

    groups, group_masks = group_rotation.make_groups(
        single_molecule, number_of_groups, residues_in_groups)

    if(simulation_type == 'single_mab'):

        simulate_single_mab(single_molecule, number_of_steps, groups, group_masks, number_of_groups, residues_in_groups, rotation_variables,
                            dihedral_parameters, flexible_residues, residue_rotation_indices, residue_rotation_mask, step_parameters, first_last_resid, cutoff)
        ...
        if(start_type == 1):
            ...
            com_coor, boxl, max_diameter_molecule = get_com_coords_and_build_box(
                single_molecule, coor_start_type, number_of_molecules, argon_file_name, argon_box_length, length_scale_factor, mab_diameter)

            print 'len(com_coor) = ', len(com_coor)
            print com_coor

            # setup_mmc.duplicate_molecule_and_assign_coor(all_molecules,single_molecule,com_coor)
            duplicate_molecule_and_assign_coor(all_molecules, single_molecule, com_coor, initial_pdb, number_of_steps, groups, group_masks, number_of_groups, residues_in_groups,
                                               rotation_variables, dihedral_parameters, flexible_residues, residue_rotation_indices, residue_rotation_mask, step_parameters, first_last_resid, cutoff)
...
if __name__ == '__main__':
    ...
    rotation_variables = [
        molecule_type, temperature, basis, number_of_ranges, reslow, numcont, dtheta]
    ...
    fc_to_fab_vector_length = ball_radius + surface1_to_anchor_cord_length + \
        maximum_anchor_to_surface2_cord_length + ball_radius
    ...
    main(simulation_type, number_of_steps, initial_pdb, output_pdb, argon_file_name, number_of_molecules, argon_box_length, length_scale_factor, ball_diameter,
         fc_to_fab_vector_length, minimum_base_angle, surface1_to_anchor_cord_length, linker_pdb, rotation_variables, number_of_groups, residues_in_groups, mab_diameter)
