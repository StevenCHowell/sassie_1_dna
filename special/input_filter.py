...
import sassie.sasmol.sasutil as sasutil
...
def check_name(filename):
	...
	for i in xrange(len(filename)):
		...
		if character in bad_characters:
			error.append('file or path : '+filename+' has incorrect character : '+character)
...
def check_exe(exe):
	...
	try:    
		...
		return error
	if(not value):
		error.append('Executable file : '+exe+' does not exist or can not be exectuted')
		return error
	...
	for key in svariables:
		...
		# print key,svariables[key][0],svariables[key][1]
			...
			try:
				...
				error.append(key+' is not a '+str(svariables[key][1])+' : '+str(svariables[key][0]))
			...
			try:
				...
				error.append(key+': could not read array of values')
				return error,variables
		elif (svariables[key][1]=='bool'):
			value=svariables.get(key)[0]
			try:
...
#       1 FALA 1    ALA  CAY  CT3   -0.270000       12.0110           0
	...
	st=string.split(infile[2]) ; num_remarks=locale.atoi(st[0]) ; print 'remarks = ',num_remarks
	offset1=2+num_remarks+2
	st=string.split(infile[offset1]) ; natoms=locale.atoi(st[0]) ; print 'natoms = ',natoms
