...
def dihedralgenerate(variables,txtOutput): 
	...
	runname,dcdfile,path,pdbfile,trials,goback,temp,molecule_type,numranges,dtheta,reslow,numcont,lowres1,highres1,basis,cutoff,lowrg,highrg,zflag,zcutoff,cflag,confile,nonbondflag,nonbondscale,psffilepath,psffilename,parmfilepath,parmfilename,plotflag,directedmc,seed=unpack_variables(variables)

	alignflag='1' ; saveflag='0' 
		...
		if(basis.lower() == 'all'):
			...
			for i in xrange(number_basis_types):
				if(i==0):
					...
					basis_filter = basis_filter + ' or name[i] == "'+string_basis[i]+'" '
...
#	ATOM N4   NN1    -0.75  !                          H2'
		...
		message='rotation basis is not setup for molecule type = '+m1.moltype()[0]
	...
	if(cflag == 1):
		...
		for i in xrange(len(distance_array)):
			...
			# testing code is below (to the end of this loop) ... remove at some point
			...
			m1.setBeta('0.00')
			value = '1.00'
			error = m1.set_descriptor_using_mask(local_mask_a_array,beta,value)
			m1.write_pdb(test_filename1,0,'w')	
			value = '0.00'
			error = m1.set_descriptor_using_mask(local_mask_a_array,beta,value)
			value = '1.00'
			error = m1.set_descriptor_using_mask(local_mask_b_array,beta,value)
	...
	if(nonbondflag==1):
		...
		charge=[];exclusionlist=[];onefourlist=[];vdwparam=[];angleparam=[];dihedralparam=[]	

		angleoutfile=open(genpaths+runname+'.angle_energies.txt','a')

	hrg=0.0 ; lowestrg=1000.0
	accepted=0 ; over=0 ; badrg=0 ; nsteps=0 ; arg=0.0 ; trg=0.0
	badz=0 ; badc=0
	...
	residue_rotation_indices,residue_rotation_mask = get_rotation_indices(m1,molecule_type,flexible_residues,txtOutput)
	name=m1.name()	
	...
	align_filter = 'name[i] == "'+basis_atom+'" and (resid[i] >= '+str(lowres1)+' and resid[i] <= '+str(highres1)+')'	
	...
	for i in range(trials):
		...
		newafile=dihedral_rotate.rotate(coor,m1,q0,th,an,cutoff,lowrg,highrg,re,accepted,zflag,zcutoff,cflag,dcdoutfile,indices,this_mask,basis_mask,sub_m2,align_mask,coor_sub_m1,com_sub_m1,mask_a_array,mask_b_array,distance_array,type_array,first_last_resid,molecule_type,cutoff_array,vdw_factor)
...
#		print 'back from dihedral_rotate'; sys.stdout.flush()
	...
	rgplot=open('./'+runname+'/generate/'+dcdfile+'.all_rg_results_data.txt','a')
	...
	rgplot=open('./'+runname+'/generate/'+dcdfile+'.accepted_rg_results_data.txt','a')
	rgplot.write('# structure number (structure 1 = 1; not 0), Rg (accepted), trial number\n')
	for ii in range(len(accepted_rg_tally)):
                rgplot.write('%i\t%f\t%i\n' % (accepted_rg_tally[ii][1]+0,accepted_rg_tally[ii][2],accepted_rg_tally[ii][0]+1))	
	rgplot.close()
			
	outfile7=open(genpaths+dcdfile+'.stats','a')
	outfile7.write('%s\t%f\t%s\t%f\n' % ('lowest Rg = ',lowestrg,'highest Rg = ',hrg))
	outfile7.write('%s\t%i\t%s\t%i\t%s\t%f%s\n' % ('accepted ',accepted,' out of ',nsteps,' moves : ',(accepted/float(nsteps))*99.0, ' %'))
	...
	if(cflag==1):
		outfile7.write('%s\t%i\t%s\t%i\t%s\t%f%s\n' % ('constraint filter rejected ',badc,' out of ',nsteps,' moves : ',(badz/float(nsteps))*100.0, ' %'))
	if(accepted>0):
