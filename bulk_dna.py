...
def rotate_coarse_grained_beads(other_self, group_number):
    ...
    #s group_all_atom_dcd_out = group_mol.open_dcd_write(group_all_atom_out_file)
    ...
    #s 
    #s # will write these out to dcd files to store the coordinates along the way
    ...
    #s handle_error(other_self, error)
    x_vec_mol.setCoor(np.array([xyz_vectors[0]])) # the np.array recast these so they 
    ...
    na_energetics_width = {'a': 0, 'b': 46., 'z': 0} # yet to use a, and z type nucleic acid
    ...
    bend_energy_old = bend_energy(other_self, persistence_length, bead_unit_vectors, mean_bead_distance)
    ...
    #s while n_accept < trials: # for calculating the Rg and Re we want every fails counted as separate structures
    for trial in xrange(trials):
        # Choose a bead to rotate
        trial_bead = np.random.randint(1, n_cg_beads) # rotating bead 0 doesn't mean anything

        # Determine rotation to perform
        theta_max_xyz = np.array([theta_max, theta_max, theta_z_max]) # degrees
        theta_xyz = np.zeros(3)
        # theta_xyz[np.random.randint(3)] = 1.0 # randomly select x, y, or z
        theta_xyz[np.random.randint(2)] = 1.0 # randomly select x, or y   # do not want to use z rotations
        theta_xyz *= theta_max_xyz * (2 * np.random.random() - 1) # continuously distribution
        ...
        (bead_unit_vectors, bead_distances) = get_chain_parameters(other_self, flex_coor)
        ...
        if test > probability:  # pass: test <= probability
            ...
            log.debug('step rejected: ds nucleic acid bending energy parameters')
        else:
            ds_nucleic_acid_pass = True
        
            error, post_heavy_coor = post_mol.get_coor_using_mask(0, post_overlap_mask)
            ...
            if 1 == two_body_overlap(other_self, flex_coor[trial_bead:], 
                ...
                log.debug('step rejected: collision between post atoms and the fixed NA beads')
                collision = 1
        ...
        if ds_nucleic_acid_pass and collision == 0:
            ...
            cg_flex_mol.setCoor(np.array([flex_coor])) # update flex coordinates
            post_mol.setCoor(np.array([post_coor]))    # update post coordinates
            nvars.n_moves[group_number] += 1              # increment the number of moves for the group
            ...
            wca_energy_matrix_old = np.copy(wca_energy_matrix_new) # update dsNA WCA energy        
            total_energy_old = total_energy_new                       # update total energy
            ...
            if fail_tally == n_go_back:
                ...
                i_go_back = go_back(other_self, n_accept, cg_flex_out_file, 
                                   ...
                                   xyz_vectors)
                flex_coor = np.copy(cg_flex_mol.coor()[0]) # get the flex_coor from the loaded file
                
                # get the reference energy for the loaded configuration
                (bead_unit_vectors, bead_distances) = get_chain_parameters(other_self, flex_coor) 
                (wca_energy_old, wca_energy_matrix_old) = get_wca_energy(other_self, chain_width, flex_coor, wca_energy_matrix_old, 0)
                bend_energy_old = bend_energy(other_self, persistence_length, bead_unit_vectors, bead_distances)
                total_energy_old =  bend_energy_old + wca_energy_old
                ...
                flex_coor = np.copy(cg_flex_mol.coor()[0]) # reset the flexible coordinates
                
            post_coor = np.copy(post_mol.coor()[0]) # reset the post coordinates
            xyz = np.copy(xyz_vectors)              # reset the bead orientations
    ...
    try:
        log.info('removing dcd file used to store coordinate vectors: %s' % x_vec_dcd_file)
        os.remove(x_vec_dcd_file)
        log.info('removing dcd file used to store coordinate vectors: %s' % y_vec_dcd_file)
        os.remove(y_vec_dcd_file) 
        log.info('removing dcd file used to store coordinate vectors: %s' % z_vec_dcd_file)
        ...
        if keep_cg_files:
            ...
            log.info('removing coarse-grained flexible nucleic acid pdb file stored for go_back: %s.pdb' % cg_flex_out_file[:-4])
            os.remove(cg_flex_out_file[:-4] + '.pdb')
            log.info('removing coarse-grained flexible nucleic acid dcd file stored for go_back: %s' % cg_flex_out_file)
            os.remove(cg_flex_out_file)
            log.info('removing post pdb file stored for go_back: %s.pdb' % post_out_file[:-4])        
            os.remove(post_out_file[:-4] + '.pdb')
            log.info('removing post dcd file stored for go_back: %s' % post_out_file)
...
def unpack_variables(other_self,variables):
        ...
        vars.number_of_flexible_regions = variables['number_of_flexible_regions'][0]
...
if __name__ == "__main__":
    ...
    log.info('iter: %d of %d; (a, r) = (  ,  ); rg/lp = %0.2f; h/lp = %0.2f' % (0, iters, rg0, h0) )
    ...
    for i in xrange(iters):
        ...
        log.info('iter: %d of %d; (a, r) = (%d, %d); rg/lp = %0.2f; h/lp = %0.2f' % (0, iters, nvars.n_accept, nvars.n_reject, rg_lp, h_lp) )
