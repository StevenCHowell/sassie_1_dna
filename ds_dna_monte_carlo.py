...
try:
    ...
    print 'failed to load `merge_dcd_files`'
    print 'likely renamed to `merge_trajectory_files`'
...
def make_cg_dna(dna_segnames, dna_resids, bp_per_bead, aa_all, txtOutput,
    ...
    error = aa_all.copy_molecule_using_mask(
        aa_dna, aa_dna_mask, frame)  # select dna
...
def make_rigid_groups(aa_all, rigid_groups):
    ...
    if len(rigid_groups) > 0:
        ...
        # masks to separate the cg_protein into NCP groups
        rigid_group_masks = []
        rigid_group_mols = []
        (pre, post, btwn) = ("((segname[i]=='", "'))", " or ")
...
def is_bead_flexible(flex_resids, nbeads, resid1, bp_per_bead, debug=False):
    # Flexible part
    # create an array (groupid) labeling the group for each flexible cg-DNA
    # bead
...
def recover_aaPro_model(aa_pgroup_masks, cg_pgroup_masks, cg_pro, all_proteins,
    ...
    for (i, mask) in enumerate(aa_pgroup_masks):
        if sum(mask) > 0:
            ...
            # define a temporary sasmol object for the original cg protein
            # group
...
def checkU(coor):
    ...
    # magnitues of u-vectors
    lu = numpy.sqrt(u[:, 0]**2 + u[:, 1]**2 + u[:, 2]**2)
    nu = lu.size
...
def energyBend(lp, u, l):
    ...
    uk0 = u[:-1, :]  # ; print 'u_k= \n', uk0
    uk1 = u[1:, :]  # ; print 'u_k= \n', uk1
    # calculate the sum of their products
    res = numpy.sum((1 - numpy.inner(uk0, uk1).diagonal()) / l[:-1])
...
def f_overlap2(coor_a, coor_b, cutoff):
    ...
    if len(coor_a) > 0 and len(coor_b) > 0:
        # calculate the distance between moved DNA beads and all other DNA
        # beads
...
def dna_mc(trials, i_loop, theta_max, theta_z_max, debug, goback, n_dcd_write,
    ...
    # the numpy.array recast these so they
    ...
    while n_accept < trials:
        ...
        thetaZ = 2 * trial_theta_z_max * \
            numpy.random.random() - trial_theta_z_max
        thetaX = 2 * trial_theta_max * numpy.random.random() - trial_theta_max
        thetaY = 2 * trial_theta_max * numpy.random.random() - trial_theta_max
        thetaXYZ = [
            thetaX / softrotation, thetaY / softrotation, thetaZ / softrotation]
        ...
        if dna_pass and not collision:
            ...
            if 0 == n_accept % n_dcd_write:
                ...
                if goback > 0:
                    # default goback is -1 so this returns FALSE without user
                    # input
...
def main(variables):
    ...
    while remaining_trials > 0:
        ...
        if openmm_min:
            # set at 100 for minimization purposes (increase)
            loop_trials = 100
            if remaining_trials < loop_trials:
                loop_trials = remaining_trials
            # increment for the number of trials
...
def minimize(aa_dcd, refpdb, path, psffile, runname, temperature,
    ...
    if openmm_min:
        ...
        energy_convergence = 1.0 * unit.kilojoule / \
            unit.mole  # the default value
        if i_loop == 0:
            ...
            variables.integrator = LangevinIntegrator(
                temperature * unit.kelvin, 1 / unit.picosecond, step_size)
            simulation = omm.initialize_system(variables)
        omm.energy_minimization(
            simulation, raw_structure, energy_convergence, max_steps)
    else:
...
def prepare_dna_mc_input(variables):
    ...
    assert len(
        theta_max) == n_flex_regions, 'theta_max should have %d values' % n_flex_regions
    assert len(
        theta_z_max) == n_flex_regions, 'theta_z_max should have %d values' % n_flex_regions
    assert len(
        first_res_per_region) == n_flex_regions, 'first_res_per_region should have %d values' % n_flex_regions
    assert len(
        n_cont_res_per_region) == n_flex_regions, 'n_cont_res_per_region should have %d values' % n_flex_regions
...
if __name__ == "__main__":
    ...
    # bending
    svariables['theta_z_max'] = ('5', 'float_array')
